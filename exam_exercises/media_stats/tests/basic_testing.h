...
//
template<class T>
bool almost_equal(T x, T y, T slack) {
    return std::fabs(x-y) <= slack * std::fabs(x+y)
        // unless the result is subnormal
        || std::fabs(x-y) < std::numeric_limits<T>::min();
}

#define CHECK_ALMOST_EQUAL(X,Y) do {					\
	auto x_ = (X);							\
	decltype (x_) y_ = (Y);						\
	if (!(almost_equal(x_,y_, 0.0001))) {				\
	    std::cout << std::endl <<__FILE__ << ":" << __LINE__	\
...
BT_POSSIBLY_UNUSED
static unsigned int bt_timeout = 5; /* seconds */
...
const char * bt_test_usage
  = "usage: %s [-d] [-q] [-t <seconds>]  [-- <test_name> ...]\n"
